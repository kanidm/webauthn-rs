...
pub fn verify_fido_sk_ssh_attestation(
    ...
    let ca_crt = verify_attestation_ca_chain(&attestation.data, attestation_cas, current_time)?;
...
impl TryFrom<&[u8]> for SshSkAttestation {
    ...
    fn try_from(data: &[u8]) -> Result<SshSkAttestation, WebauthnError> {
        ...
        let att_cert = x509::Certificate::from_der(sk_raw.att_cert_raw)
            .map_err(|_err| WebauthnError::X509DerInvalid)?;
